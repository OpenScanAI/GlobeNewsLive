...
const TYPE_CONFIG: Record<
  string,
  { icon: string; color: string; label: string; priority: number }
> = {
  surveillance: {
    icon: "🔍",
...
export default function FlightRadar() {
  ...
  const { data, isLoading, error, mutate, isValidating } = useSWR<{
    ...
    simulated?: boolean;
  }>(`/api/flights?region=${region}&military=${!showAll}`, fetcher, {
  ...
  // Emergency squawks
  const emergencyAircraft = aircraft.filter(
    (a) => a.squawk && EMERGENCY_SQUAWKS.includes(a.squawk),
  );
  ...
  const typeStats = Object.entries(data?.categories || {})
    ...
    .sort(
      (a, b) =>
        (TYPE_CONFIG[a[0]]?.priority || 99) -
        (TYPE_CONFIG[b[0]]?.priority || 99),
    );
    ...
    <div className="glass-panel overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {data?.simulated && (
            <span
              className="text-[8px] px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-400 font-mono"
              title="API unavailable - showing simulated data"
            >
              ⚠️ SIM
            </span>
          )}
          {/* Real-time indicator */}
          <span
            className={`flex items-center gap-1 text-[8px] px-1.5 py-0.5 rounded font-mono ${
              isValidating
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${isValidating ? "bg-green-400 animate-pulse" : "bg-gray-500"}`}
            />
        ...
        <div className="space-y-0">
          ...
          <div className="flex items-center justify-between px-2 py-1.5 border-b border-border-subtle bg-black/20">
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() =>
                  setViewMode(viewMode === "list" ? "stats" : "list")
                }
            ...
            <div className="text-[9px] text-text-dim font-mono">
              {selectedRegion?.icon} {data?.total || 0} aircraft •{" "}
              {data?.military || 0} military
            </div>
          </div>
                ...
                {criticalAircraft.slice(0, 3).map((a) => (
                  ...
                  >
                    ...
                    <span className="text-text-dim">
                      FL{Math.round(a.altitude / 100)} • {a.speed}kts •{" "}
                      {a.heading}°
                    </span>
                  </div>
                ))}
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {isLoading && aircraft.length === 0 ? (
                ...
                <div className="text-center py-8 text-[10px] text-text-muted">
                  No {typeFilter || "military"} aircraft in{" "}
                  {selectedRegion?.name}
                </div>
              ) : (
                filteredAircraft.slice(0, 25).map((a) => {
                  const config = TYPE_CONFIG[a.type] || TYPE_CONFIG.military;
                  const isCritical = CRITICAL_CALLSIGNS.some((cs) =>
                    a.callsign.toUpperCase().startsWith(cs),
                  );
                  const isEmergency =
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setSelectedAircraft(
                          selectedAircraft === a.id ? null : a.id,
                        )
                      }
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[9px] ${config.color}`}
                          >
                            {config.icon}
                          </span>
                          <span
                            className={`font-mono text-[11px] ${isCritical ? "text-purple-400 font-bold" : "text-white"}`}
                          >
                            {a.callsign || "UNKNOWN"}
                          </span>
                          {isCritical && (
                      ...
                      {selectedAircraft === a.id && (
                        <div className="mt-2 pt-2 border-t border-border-subtle grid grid-cols-2 gap-2 text-[9px]">
                          ...
                          <div>
                            ...
                            <span
                              className={`ml-1 ${config.color.split(" ")[0]}`}
                            >
                              {config.label}
                            </span>
                          </div>
                          ...
                          <div>
                            ...
                            <span
                              className={`ml-1 ${a.verticalRate > 0 ? "text-green-400" : a.verticalRate < 0 ? "text-red-400" : "text-white"}`}
                            >
                              {a.verticalRate > 0
                                ? "↑"
                                : a.verticalRate < 0
                                  ? "↓"
                                  : "—"}{" "}
                              {Math.abs(a.verticalRate)} fpm
                            </span>
                          </div>
                          <div>
                            <span className="text-text-dim">Squawk:</span>
                            <span
                              className={`ml-1 font-mono ${isEmergency ? "text-red-400 font-bold" : "text-white"}`}
                            >
                              {a.squawk || "N/A"}
                            </span>
                          </div>
          ...
          <div className="px-3 py-1.5 bg-black/30 border-t border-border-subtle flex items-center justify-between">
            ...
            <div className="text-[8px] text-text-dim">
              ...
              {data?.simulated && (
                <span className="text-yellow-400 ml-1">• DEMO</span>
              )}
            </div>
          </div>
        </div>
